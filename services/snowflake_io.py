...
def build_snowflake_connection(database: Optional[str] = None):
    """
...
# ─── Streamlit-cached connection (with auto-reconnect) ──────────────
# The app and all analytics/services modules use THIS entry point; the
# raw builder above is for scripts that run outside Streamlit (batch
# tools import it via the snowflake_auth shim).
...
def _create_snowflake_connection():
    ...
    return build_snowflake_connection()
