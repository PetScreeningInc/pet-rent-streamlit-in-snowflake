...
Trade-offs:
  - SLOW: 2 to 4 SOAP calls per property (charges + 3 resident-status calls).
    Use the ThreadPoolExecutor pool (max 8) to keep wall time reasonable.
  - Returns ONLY the current snapshot — there is no historical postmonth
    backfill. ``charge_from_date`` is the first of the current month,
    ``charge_to_date`` is clipped to min(bill_end, move_out, today).
"""
...
def _credentials() -> Tuple[str, str, str]:
    user = os.getenv("ONESITE_USERNAME", "").strip()
    pwd = os.getenv("ONESITE_PASSWORD", "").strip()
    lic = os.getenv("ONESITE_LICENSE_KEY", "").strip()
    if not (user and pwd and lic):
        raise RuntimeError(
            "Missing OneSite credentials. Set ONESITE_USERNAME, "
            "ONESITE_PASSWORD and ONESITE_LICENSE_KEY in the environment."
        )
    return user, pwd, lic
...
def _fetch_property(
    ...
    # Build the workload: 1 charges fetch + 3 resident fetches.
    ...
    def _fetch_residents(status: str) -> Tuple[str, Any]:
        body = _build_residents_body(
            pmc_id, site_id, status, username, password, license_key
        )
        text = _post_with_retry(
            body, SOAP_ACTION_RESIDENTS, prop_label=str(prop_label)
        )
        return (f"residents_{status}", _parse_residents_response(text or "", status))

    errors: List[str] = []
    with ThreadPoolExecutor(max_workers=MAX_PER_PROP_WORKERS) as ex:
        futs = [ex.submit(_fetch_charges)]
        for st in RESIDENT_STATUSES:
            futs.append(ex.submit(_fetch_residents, st))
